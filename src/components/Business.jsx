...
const TestFeatureCard = ({ content, title, icon }) => {
  return (
    <div className="group flex flex-col ">
      <div className="relative flex flex-start   w-[25rem] md:w-[33rem]  h-[5rem] rounded-full my-[1rem] items-center transition-all duration-500 ease-in-out transform group-hover:scale-110">
        <div className={`${styles.flexCenter} w-[3rem] h-[3rem] bg-dimBlue rounded-full ml-[12px]`}>
          <img
            src={icon}
            alt=""
            className="w-[2rem] h-[2rem]  filter invert brightness-max"
          />
        </div>
  ...
  return (
    <div className= {`flex md:flex-row flex-col ${styles.main} relative`} >
      <div className={`text-white flex flex-col font-poppins`}>
        <p className={`font-bold text-5xl mt-9 py-1 leading-snug md:leading-none`}>
          
            <span>Enjoy Your Games</span>
         
          <br className="sm:block hidden" />
          <div className="mt-7 ">
            <span className="bg-gradient-to-r from-purple-500 via-pink-500 to-red-500 text-transparent bg-clip-text">We&apos;ll Handle The Money!</span>
          </div>
        </p>
        <p className={`${styles.paragraph}  font-thin mt-7 max-w-[640px]`}>
        Welcome to our innovative bank, crafted exclusively for gamers like you. Elevate your gaming experience while we take care of your financial needs. Earn credits, unlock bonuses, and enjoy exciting discounts on your favorite games. 
        </p>
